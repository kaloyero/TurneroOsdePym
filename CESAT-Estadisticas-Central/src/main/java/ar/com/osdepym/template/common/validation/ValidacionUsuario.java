...
public class ValidacionUsuario {
	...
	public Usuario execute(String user, String passw, Usuario usuario) {
		...
		try {
			...
			String query = "SELECT * from cesat.usuario  WHERE nom_usuario = ? and  password like ? and habilitado like 'SI' ";
			...
			while (rs.next()) {
				...
				if (existe && perfil!=2) {
					usuario.setCod_error(4);
					usuario.setMensaje_error("El usuario no tiene los privilegios para acceder a este modulo");
